...
final class AIChatRepository {
    ...
    func createAIChat(audio: StoreAudioModel) -> AnyPublisher<ConversationModel, Error> {
      networkService.performRequest(route: AIChatRequest(endpoint: .createAIChat(audio: audio)))
            .map { value in
                return value
            }
            .catch { error -> AnyPublisher<ConversationModel, Error> in
                return Fail(error: error).eraseToAnyPublisher()
            }
            .eraseToAnyPublisher()
    }
    
    func getAnswer(messageId: String) -> AnyPublisher<ConversationModel, Error> {
    ...
    func addNewMessage(threadId: String, audio: StoreAudioModel) -> AnyPublisher<ConversationModel, Error> {
    ...
    func getAllChat() -> AnyPublisher<[ConversationModel], Error> {
